...
- **Kagi provider (`kagi`)**: a fifteenth built-in Provider supplying Search and Diagnostics. Search runs against `GET https://kagi.com/api/v1/search` with `Authorization: Bot <key>` (Kagi wire truth — not Bearer); `--domain` maps to a `site:` operator prefixed onto the query and `--topic news` routes to `GET https://kagi.com/api/v0/enrich/news`, while `--recency`, `--location`, `--content-size`, and `--type` are rejected with `UNSUPPORTED_OPTION` before any transport call. The `{ meta, data: [...] }` envelope is filtered to `t === 0` rows carrying a url — `t: 1` rows are related-query suggestions and are dropped; a non-array `data` fails closed rather than degrading to an empty success. The diagnostics probe issues a single `limit=1` search GET and normalizes failures through the shared probe-error seam (401 → `CONFIGURATION_ERROR`, 403 → quota). Credentials: `KAGI_API_KEY` wins over the legacy `KAGI_TOKEN`; whitespace-only values count as absent. Kagi advertises no quota capability, so its quota authority is `always-unknown`. Wired into the static registry, the init wizard, the controls-conformance table (six search control rows), the search-conformance factory, and the provider docs.
