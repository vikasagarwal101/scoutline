...
- **`usage.json` ledger:** every billable invoke — `search` (fan-out arms and `--merge` sub-queries each record their own invoke), `read`, `crawl`, `map`, `research`, `repo`, and `vision` — now appends counters to `<config-root>/usage.json` (sibling of `config.json`; `SCOUTLINE_CONFIG_DIR` moves it, cache-dir variables do not). Rows are keyed by UTC calendar day → provider → capability id and hold `attempts` / `firstTries` / `exactUnits` (reserved, 0 today) / `estimateUnits` / `unknownCount`; retries each count as an attempt and cache hits record nothing. Writes are lock-serialized (`usage.json.lock`) read-modify-write cycles applied as atomic temp-file renames; retention is 90 days, pruned on the first write of a new UTC day; a write failure degrades to a redacted warning (no event detail) and never fails the command. The ledger stores counters only — never queries, URLs, prompts, results, or credentials — and a corrupt or version-mismatched file is treated as empty (fail-open) rather than blocking the CLI.
- **`scoutline usage [--days N] [--provider <id>]`:** new credential-free early-dispatch command (same short-circuit class as `cache`) that folds the ledger into a schema-v1 envelope — `windowDays`, `generatedAt`, `unitNote`, per-provider `totals` with ascending per-capability rows and providers ascending; data mode emits the envelope, text modes render a fixed-column table of the same data. Default window is 7 UTC days; `--days` must be an integer ≥ 1 (`--days 0` / non-numeric → `VALIDATION_ERROR`), and an unknown `--provider` id exits 1 listing the accepted ids while a known-but-unrecorded id reports an empty window. A missing or corrupt ledger reports an empty window with exit 0 — never stderr noise.
