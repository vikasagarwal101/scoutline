...
- **Bounded response consumption across all science provider clients (issue #150):** hardened five keyless science clients (`arxiv`, `crossref`, `europepmc`, `openalex`, `pubmed`) against unbounded memory accumulation across six total consumption paths (five ok-path reads including PubMed's two-step `esearch.fcgi` JSON and `efetch.fcgi` XML, plus the non-ok response drain). Each client now enforces a Content-Length header pre-check against the shared 50MB `MAX_BUFFERED_RESPONSE_BYTES` ceiling from `src/lib/bounded-body.ts` (cancelling the body and throwing a terminal 413 `ApiError`), consumes bodies incrementally via `readBoundedResponseBody` with active byte tracking and reader cancellation on breach, and replaces the buffered `res.text().catch(...)` non-ok drain with immediate zero-buffering connection release via `res.body?.cancel().catch(...)`.
