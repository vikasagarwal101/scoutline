...
- **Review fixes (PR #33):** `config` now short-circuits before the credentialed config load — a corrupt `config.json` never blocks `config get/set/unset`, and the dispatcher's ambient-env inspection can never diverge from the file the command acts on. `config get routing.<typo>` fails with `Unknown capability` instead of printing a silent `(not set)`. Typed `set`/`unset` no longer suppress write-time config warnings. Skill docs pin `npx scoutline@<version>` examples (unversioned npx fetches untrusted latest from the registry); README/roadmap provider-quota and shipped-feature references re-synced.
