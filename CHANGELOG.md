...
- **Z.AI MCP init probe gated on failure provenance (issue #128):** the failure-path auth probe now runs ONLY for the registerManual-failure ApiError (sentinel-tagged at construction); a factory/transport-thrown `ApiError` during initialization fails fast with its original status and message — zero probe network requests — instead of risking reclassification by an unrelated 401/403 probe answer. RegisterManual failures keep the #117 behavior (one bounded probe, AUTH_ERROR classification).
