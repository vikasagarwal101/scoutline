...
6. Repository exploration is a Provider Capability. Participation in selection follows descriptor metadata: only the Z.AI descriptor advertises `repository-exploration`; the others do not. Provider fallback is **always-on by default** (0.11.0+): selecting any non-supplier emits a stderr notice and silently reroutes to Z.AI. The preflight still runs capability metadata → configuration → adapter handle on the effective provider in order, so the strict ordering is preserved. **Do not add an additional implicit fallback path beyond the executor** — `--no-fallback` (or `SCOUTLINE_NO_FALLBACK=1`) is the documented opt-out that restores the previous `UNSUPPORTED_CAPABILITY` behavior under the same preflight.
...
8. Reader is a Provider Capability. Participation in selection follows descriptor metadata: Z.AI, Tavily, Exa, and Firecrawl advertise `reader`; MiniMax and Brave do not. Provider fallback is **always-on by default** (0.11.0+): selecting a non-supplier emits a stderr notice and silently reroutes to the next eligible Provider. The preflight still runs capability metadata → configuration → adapter handle on the effective provider in order. **Do not add an additional implicit fallback path beyond the executor** — `--no-fallback` (or `SCOUTLINE_NO_FALLBACK=1`) is the documented opt-out that restores the previous `UNSUPPORTED_CAPABILITY` behavior under the same preflight. Reader has no Explorer module — projection lives in the thin `commands/read.ts` handler.
9. Reader results are schema-version-1 structured values (`{schemaVersion, url, finalUrl, ...}`). The breaking migration from v0.2 raw content strings and bare extract arrays is intentional. The four `--extract` modes and their item shapes are unchanged from v0.2. `--full-envelope` is silently deprecated; `--max-chars` is a whole-envelope Output Budget on both shapes (extract reads trim field values only and report `originalItemCount`).
