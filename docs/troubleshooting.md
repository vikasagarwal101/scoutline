...
```
Unknown provider "<value>". Accepted provider IDs: zai, minimax, tavily, exa, brave, firecrawl, parallel, perplexity, jina, you, linkup, spider, bocha, searchapi, kagi, arxiv, openalex, crossref, pubmed, europepmc.
```

`--provider` and `SCOUTLINE_PROVIDER` accept `zai`, `minimax`, `tavily`,
`exa`, `brave`, `firecrawl`, `parallel`, `perplexity`, `jina`, `you`,
`linkup`, `spider`, `bocha`, `searchapi`, `kagi`, `arxiv`, `openalex`, `crossref`, `pubmed`, or
