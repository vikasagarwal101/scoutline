...
```bash
scoutline config get                        # full config, credentials always masked
scoutline config set routing.search tavily,brave   # strict: typos FAIL, not drop
scoutline config set routing.reader parallel,linkup,jina   # first configured reader supplier wins
...
3. `scoutline config set fanout true` (no pin) — standing
...
| Capability | Z.AI | MiniMax | Tavily | Exa | Brave | Firecrawl | Parallel | Perplexity | Jina AI | You.com | Linkup | Spider.cloud | Bocha AI | SearchApi | arXiv | OpenAlex | Crossref | PubMed | Europe PMC | Command |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Search | Yes | Yes (no domain/recency/content-size/location) | Yes (no location) | Yes (no location) | Yes (web/news/video; `--content-size high` → LLM Context) | Yes (no location; `--content-size high` = markdown, +1 credit/result) | Yes (domain, recency, location, content-size via `advanced_settings`; topic via keyword) | Yes (domain, recency, content-size; topic via keyword) | Yes (domain, location; rejects recency/content-size; topic via keyword) | Yes | Yes | Yes | Yes | Yes | No | No | No | No | No | `scoutline search` |
| General single-image interpretation | Yes | Yes (JPG/JPEG/PNG/WebP ≤50 MiB) | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | `scoutline vision analyze` |
| Specialized Vision (UI-to-code, OCR, error diagnosis, diagram) | Yes | Available (live-attested; conformance-gated) | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | `scoutline vision ui-to-code`, `vision extract-text`, `vision diagnose-error`, `vision diagram` |
| Specialized Vision (chart) | Yes | Pending (implemented; fixture image defect blocks live conformance) | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | `scoutline vision chart` |
| Two-image diff, video | Yes | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | `scoutline vision diff`, `vision video` |
| Quota (normalized) | Yes | Yes | Yes | **No** (deferred) | Yes (rate-limit window, not spend) | Yes (credits) | **No** | **No** | Yes (rate-limit telemetry, not spend) | **No** | Yes (credit balance, not spend) | Yes (credit balance, not spend) | No | Yes | No | No | No | No | No | `scoutline quota [--all-providers]` |
| Diagnostics | Yes | Yes | Yes | Yes | Yes | Yes (single-scrape probe) | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes (keyless probe) | Yes (keyless probe) | Yes (keyless probe) | Yes (keyless probe) | Yes (keyless probe) | `scoutline doctor [--no-tools] [--available]`; every row carries `availability` (`ok`/`exhausted`/`error`/`unconfigured`), rows sort healthy-first, and `availableProviders` lists the `ok` providers in registry order |
| Reader | Yes | **No** (UNSUPPORTED_CAPABILITY) | Yes (rejects Z.AI-only options) | Yes (rejects Z.AI-only options) | **No** (UNSUPPORTED_CAPABILITY) | Yes (returns page titles) | Yes | **No** (UNSUPPORTED_CAPABILITY) | Yes | Yes (rejects Z.AI-only options plus `--format text`) | Yes (renders JavaScript; rejects `--format text` and `--no-images`) | Yes (rejects Z.AI-only options) | No | No | No | No | No | No | No | `scoutline read` |
| Repository exploration (search/read/tree/brief) | Yes | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | **No** (UNSUPPORTED_CAPABILITY) | No | No | No | No | No | No | No | `scoutline repo ...` |
| Crawl | **No** | **No** | Yes | **No** | **No** | Yes (async; resumable after Ctrl-C) | **No** | **No** | **No** | **No** | **No** | Yes (sync) | No | No | No | No | No | No | No | `scoutline crawl` |
| Map | **No** | **No** | Yes | **No** | **No** | Yes | **No** | **No** | **No** | **No** | **No** | Yes | No | No | No | No | No | No | No | `scoutline map` |
| Research (4-250 credits) | **No** | **No** | Yes | Yes | **No** | **No** (`/deep-research` deprecated) | Yes | Yes | Yes | Yes | Yes | **No** | No | No | No | No | No | No | No | `scoutline research` |
| Science (scholarly search/get) | No | No | No | No | No | No | No | No | No | No | No | No | No | No | Yes | Yes | Yes | Yes | Yes | `scoutline science search <query>` / `scoutline science get <identifier>`; keyless — no API key required; controls `--author`/`--year`/`--venue`/`--type` consumed only where the supplier wire supports them |
| Raw tools | Yes | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | `scoutline tools`, `tool`, `call` |
| Code Mode | Yes | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | No | `scoutline code ...` |
...
bypasses for one invocation); transient failures (timeouts, network
errors, 5xx) retry once with backoff — supplier 429s surface as terminal
quota errors. Cache-served runs record journal warm-repeat markers.
...
```bash
# Z.AI (default)
scoutline vision analyze ./screenshot.png "What errors do you see?"
scoutline search "React 19 new features" --count 5
scoutline read https://docs.example.com/api
scoutline read https://docs.example.com/api --with-images-summary --no-gfm
scoutline fetch https://api.github.com/repos/nodejs/node --sha256
scoutline fetch https://example.com/report.pdf --pdf text --out report.txt
scoutline archive cdx https://example.com/ --from 20230101 --to 20231231
scoutline archive get https://example.com/ --at 20230601000000 --raw
scoutline archive diff https://example.com/ --since 30d
scoutline watch add https://example.com/changelog --name example-changelog
scoutline watch run --all        # exit 0/1/2: no-change / change / fetch error
...
scoutline --provider minimax vision analyze ./diagram.png "Explain this"
...
scoutline --provider tavily crawl https://docs.example.com --depth 2
scoutline --provider tavily map https://docs.example.com
...
scoutline --provider exa search "latest AI research" --topic news
scoutline --provider exa read https://example.com/
scoutline --provider exa research "Compare Rust async runtimes"
scoutline doctor --provider exa

# Brave (Search: web, news, video)
scoutline --provider brave search "AI policy news" --topic news
scoutline --provider brave search "rust async" --type video
scoutline --provider brave search "large context topic" --content-size high
scoutline --provider brave quota
scoutline doctor --provider brave

# Firecrawl (Search, Reader, Crawl, Map)
scoutline --provider firecrawl search "AI funding rounds" --content-size high
scoutline --provider firecrawl read https://example.com/
scoutline --provider firecrawl crawl https://docs.example.com --depth 2
scoutline --provider firecrawl map https://docs.example.com
scoutline --provider firecrawl quota   # remaining credits

# Parallel AI (Search, Reader, Research)
scoutline --provider parallel search "AI funding rounds" --topic news
scoutline --provider parallel read https://example.com/
scoutline --provider parallel research "Compare Rust async runtimes"
scoutline doctor --provider parallel

# Perplexity (Search, Research)
scoutline --provider perplexity search "latest AI research" --topic news
scoutline --provider perplexity research "Compare Rust async runtimes"
scoutline doctor --provider perplexity

# Jina AI (Search, Reader, Research)
scoutline --provider jina search "AI policy news" --topic news
scoutline --provider jina read https://example.com/
scoutline --provider jina research "State of carbon capture 2025"
scoutline --provider jina quota   # rate-limit telemetry, not spend
...
scoutline doctor --provider you

# Linkup (Search, Reader, Research)
scoutline --provider linkup search "AI funding rounds" --topic news
scoutline --provider linkup read https://example.com/
scoutline --provider linkup research "Compare Rust async runtimes"
scoutline --provider linkup quota   # prepaid USD balance
scoutline doctor --provider linkup

# Spider.cloud (Search, Reader, Crawl, Map)
scoutline --provider spider search "AI funding rounds" --topic news
scoutline --provider spider read https://example.com/
scoutline --provider spider crawl https://docs.example.com --depth 2
...
scoutline history recall "rust async" --limit 5
...
scoutline cache clear                 # delete every file in cache/ and tools/
scoutline cache prune --older-than 24h   # delete entries older than 24h (Nh|Nm|Ns|seconds)
