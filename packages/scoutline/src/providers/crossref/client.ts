...
export async function fetchCrossrefJson(
  ...
  try {
    ...
    if (contentLengthHeader && Number(contentLengthHeader) > MAX_BUFFERED_RESPONSE_BYTES) {
      await res.body?.cancel().catch(() => {});
      throw new ApiError(
        `Crossref response exceeds the 50MB in-memory ceiling (${contentLengthHeader} bytes declared) — refusing to buffer`,
        413,
      );
    }
    try {
      if (res.body) {
        ...
        try {
          ...
          if (err instanceof ValidationError) {
            throw new ApiError(
              "Crossref response exceeds the 50MB in-memory ceiling (stream exceeded it mid-read) — refusing to buffer",
              413,
            );
          }
          throw err;
