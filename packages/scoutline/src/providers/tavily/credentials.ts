/**
 * Tavily credential resolution (DESIGN.md §6, tech-plan §7).
 *
 * Single source of truth for the Tavily API key. Whitespace-only values
...
export const ENV_NAMES = ["TAVILY_API_KEY"] as const;

const MISSING_KEY_HELP = 'export TAVILY_API_KEY="your-api-key"';
...
/**
 * Resolve the Tavily API key without throwing. Returns `undefined`
 * when no non-blank value is present.
 */
export function resolveTavilyApiKey(env: NodeJS.ProcessEnv): string | undefined {
  return pickNonBlank(env.TAVILY_API_KEY);
}

/**
 * Resolve the Tavily API key or throw {@link ConfigurationError}
 * (exit 3) when it is missing. Call this at every capability
 * invocation gate.
 */
export function requireTavilyApiKey(env: NodeJS.ProcessEnv): string {
  const key = resolveTavilyApiKey(env);
  if (key === undefined) {
...
/**
 * True when a non-blank Tavily API key is configured. Metadata-only:
 * performs no transport construction and reads no other Provider's
 * credentials.
 */
export function isTavilyConfigured(env: NodeJS.ProcessEnv): boolean {
  return resolveTavilyApiKey(env) !== undefined;
}
