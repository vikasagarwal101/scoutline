...
describe("Crossref retry-hint seam — no documented emitter; no-op pin (#186 P2)", () => {
  // GROUND: Crossref documents NO reset-delay header — its `X-Rate-Limit-*` family carries interval/limit, a different family. NO-OP pin: the status-map site consults the parser and contributes nothing when no hint header is present (field absent, error byte-identical).
  //
  ...
  function hintDescriptor(status, headers) {
    return createCrossrefDescriptor({
      transport: {
