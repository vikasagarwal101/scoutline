...
describe("openalex bounded response execution hardening (#150)", () => {
  it("pre-read rejection: content-length > 50MB ceiling rejects with terminal 413 ApiError and cancels body without reading", async () => {
    ...
    await assert.rejects(
      ...
      (err) => {
        ...
        assert.match(err.message, /openalex response exceeds.*50MB.*refusing to buffer/i);
        return true;
      },
    );
