...
describe("save-artifacts T3: --save/--save-format/--save-force global flags", () => {
  ...
  it("runs the documented master-only markdown form: --save --save-format markdown", async () => {
    ...
    // Recording descriptor: capture the exact request so the consumed
    // --save-format value can be pinned out of the command positionals.
    ...
    try {
      ...
      assert.strictEqual(status, 0, `stderr=${JSON.stringify(stderr)}`);
      assert.deepStrictEqual(log, ["zai"], "the search behavior must run");
      // Review round 5 (greptile P1): the consumed --save-format value
      // must never leak into the command positionals — the request must
      // be identical to the same invocation without --save.
      const baseline = requests.at(-1);
      requests.length = 0;
      const status2 = await main(["search", "q", "--save-format", "markdown"], deps);
      assert.strictEqual(status2, 0, `stderr=${JSON.stringify(stderr)}`);
      assert.deepStrictEqual(requests.at(-1), baseline, "request must not change with a valueless --save prefix");
